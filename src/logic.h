/**************************************************************************************
 * Inline functions for bit-set logic. Note that all indices are interpreted as
 * Euclidean modulo 64 values.
 **************************************************************************************/
#ifndef LOGIC_H_
#define LOGIC_H_ 1
#include <type_traits>
...
namespace emath {
	...
	inline unsigned char cardinality (const T operand) {
		return __popcnt64((unsigned long long) operand);//WARNING: changed by me (Christoph) without deeper research
	}
	template<>
	inline unsigned char cardinality (const unsigned long operand) {
		return __popcnt(operand);//WARNING: changed by me (Christoph) without deeper research
	}
	template<>
	inline unsigned char cardinality (const unsigned int operand) {
		return __popcnt16(operand);//WARNING: changed by me (Christoph) without deeper research
	}
	...
	inline signed char msb_index (const unsigned long operand) {
		return (operand == 0) ? -1 : bitSize(unsigned long) - 1 - __lzcnt(operand); //WARNING: changed by me (Christoph) without deeper research 
	}
	template<>
	inline signed char msb_index (const unsigned int operand) {
		return (operand == 0) ? -1 : bitSize(unsigned int) - 1 - __lzcnt(operand); //WARNING: changed by me (Christoph) without deeper research 
	}
	...
	inline signed char lsb_index (const unsigned long operand) {
		return (operand == 0) ? -1 : __lzcnt(operand); //WARNING: changed by me (Christoph) without deeper research 
	}
	template<>
	...
	inline uint64_t reverse (uint64_t value) {
		...
		return _byteswap_uint64(value); //WARNING: changed by me (Christoph)
	}
	inline uint32_t reverse (uint32_t value) {
		...
		return _byteswap_ulong(value); //WARNING: changed by me (Christoph)
	}
	inline uint16_t reverse (uint16_t value) {
		...
		return _byteswap_ushort(value);  //WARNING: changed by me (Christoph)
	}
